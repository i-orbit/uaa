 ...
 * @since 2023/6/29
 */
public interface AssociationUserAndPositionService {
    ...
    void save(Long user, Collection<AssociationUserAndPosition> associations);

    default List<AssociationUserAndPosition> findByUser(Long user) {
        return findByUsers(Collections.singleton(user)).getOrDefault(user, Collections.emptyList());
    }

    Map<Long, List<AssociationUserAndPosition>> findByUsers(Collection<Long> users);
